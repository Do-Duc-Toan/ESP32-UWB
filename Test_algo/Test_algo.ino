...
void setup()
{
    ...
    Serial.println("Anchor_1,Anchor_2,Anchor_3,Anchor_4,x_estimated,y_estimated");
...
if (anchorCount == 4)
{
    ...
    x_est = (pow(anchorRanges[0], 2) - pow(anchorRanges[1], 2) + 9)/6;
    y_est = (pow(anchorRanges[0], 2) - pow(anchorRanges[3], 2) + 9)/6;  // Fix index
    ...
    String rangesOutput = String(anchorRanges[0] > 0 ? anchorRanges[0] : 0) + "," +
                          ...
                          String(x_est, 2) + "," +   // Add x_est to the output string
                          String(y_est, 2);          // Add y_est to the output string

    Serial.println(rangesOutput);  // Output all data to serial monitor
